...
> [!IMPORTANT]  
> **Windowsユーザーの方へ: 最も簡単なセットアップ方法**
>
> 以下の「クイックセットアップ」を利用すれば、手動でのツール配置や環境構築は不要です。
>
> 1. 空のフォルダを作成します（例：デスクトップに `my-voice-vox` フォルダなど）。
> 2. そのフォルダの中に、[install.bat](../install.bat) をダウンロードして配置します。
> 3. `install.bat` をダブルクリックして実行します。
> 4. 自動的に必要なすべてのツール（Python, FFmpeg, VOICEVOX, 各種モデル）がダウンロード・設定されます。
> 5. 完了後、作成された `run.bat` を実行すればアプリが起動します。
...
3. 以下のコマンドを実行します（Windows/Mac 共通）。  
   初回起動時は、実行環境の構築（Python やライブラリのダウンロード）が行われるため数分かかります。  
