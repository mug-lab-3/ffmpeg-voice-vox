...
class TestConfigSync(unittest.TestCase):
    ...
    def test_synthesis_update_returns_full_config_lsync(self):
        ...
        self.assertIn("synthesis", data["config"])
        self.assertEqual(data["config"]["synthesis"]["speed_scale"], 1.25)
    ...
    def test_ffmpeg_update_returns_status_only_hsync(self):
        ...
        res = self.client.post("/api/config/ffmpeg", json={"ffmpeg_path": "Z:/ffmpeg.exe"})
        self.assertEqual(res.status_code, 200)
