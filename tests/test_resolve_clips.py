...
class TestResolveClips:
    ...
    def test_get_text_plus_clips(self, mock_is_avail, mock_ensure, mock_proc_cls):
        ...
        # Mock Clips
        clip1 = MagicMock()
        clip1.GetClipProperty.side_effect = lambda p: (
            "Text+" if p == "Type" else ("" if p == "File Path" else "MyClip1")
        )

        clip2 = MagicMock()
        clip2.GetClipProperty.side_effect = lambda p: (
            "Fusion" if p == "Type" else ("" if p == "File Path" else "MyClip2")
        )

        clip_non_text = MagicMock()
        clip_non_text.GetClipProperty.side_effect = lambda p: (
            "Video"
            if p == "Type"
            else ("C:/test.mp4" if p == "File Path" else "VideoClip")
        )
    ...
    @patch("app.core.resolve.ResolveClient._ensure_connected", return_value=True)
    @patch("app.core.resolve.ResolveClient.is_available", return_value=True)
    def test_insert_file_error_when_missing(
        self, mock_is_avail, mock_ensure, mock_proc_cls
    ):
        client = ResolveClient()
        mock_resolve = MagicMock()
        client.resolve = mock_resolve
