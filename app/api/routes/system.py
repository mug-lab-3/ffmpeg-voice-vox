...
from app.web.routes import capture_service
...
@system_bp.route("/api/transcription/devices", methods=["GET"])
def get_audio_devices():
    try:
        result = get_audio_devices_handler(capture_service)
        return jsonify(result.model_dump())
    except ValueError as e:
        return jsonify({"status": "error", "message": str(e)}), 400


@system_bp.route("/api/transcription/models", methods=["GET"])
def get_available_models():
    from app.services.system_service import get_available_models_handler
    return jsonify(get_available_models_handler())
